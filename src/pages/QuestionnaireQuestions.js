...
export default function QuestionnaireQuestions() {
  ...
  const isRankingValid = useCallback(
    (promptId) => {
      const values = Object.values(rankings[promptId] || {});
      if (values.some((value) => !value)) return false;
  ...
  const handleSuggestionClick = (text, promptId) => {
  ...
  const handleSendChat = async () => {
    ...
    try {
      ...
      setChatMessages((prev) => ({
        ...prev,
        [selectedPromptId]: [...(prev[selectedPromptId] || []), assistantMessage]
      }));
    } catch (error) {
      const assistantMessage = {
        role: "assistant",
  ...
  const handleFinalSubmit = () => {
    alert("The questionnaire is closed. Responses are no longer being accepted.");
  };
      ...
      <div className="questionnaire-content">
        ...
        {stage === 2 && (
          <div className="questionnaire-questions-stage">
                  ...
                  <div className="questionnaire-suggestion-list">
                    {SUGGESTED_PROMPTS.map((prompt, index) => (
                      <button
                        ...
                        onClick={() => handleSuggestionClick(prompt.text, prompt.id)}
                      >
                ...
                <div className="questionnaire-chat-shell">
                  ...
                  <div className="questionnaire-chat-messages" ref={chatScrollRef}>
                    {selectedChat.length === 0 ? (
                      ...
                      selectedChat.map((chatMessage, index) => (
                        <div
            ...
            <div className="questionnaire-footer-actions">
              <p className="questionnaire-warning">
                The questionnaire is now closed. Responses are no longer being accepted.
              </p>

              <button
                className="questionnaire-continue-button questionnaire-disabled-button"
                type="button"
